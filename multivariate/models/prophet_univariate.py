...
def prophet_univariate(csv_path, n_years=9):
    try:
        df = pd.read_csv(csv_path)
        df.columns = df.columns.str.strip()
        df = df.drop(['Per_Capita_in_USD', 'Percentage_Growth'], axis=1)
    df_univariate.rename(columns={'GDP (current US$)': 'GDP_In_Billion_USD'}).to_csv("multivariate/data/univariate_gdp.csv", index=False)
